...
use tdoc::{parse, write};

fn main() {
    let matches = Command::new("fmtftml")
        .version("0.1.0")
        .about("Format and prettify FTML documents")
        .arg(
            Arg::new("INPUT")
        ...
        .arg(
            Arg::new("OUTPUT")
                .help("Output FTML file (use '-' for stdout)")
                .default_value("-")
                .index(2),
        )
        .get_matches();
    ...
    // Write formatted FTML
    match write(&mut writer, &document) {
        Ok(()) => {}
        Err(e) => {
