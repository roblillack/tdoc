...
fn escape_markdown_text(text: &str, line_start: bool, is_final_chunk: bool) -> String {
    ...
    while idx < chars.len() {
        ...
        match ch {
            ...
            // must always be escaped: code spans, emphasis, links, strikethrough, and the
            // escape character itself. Block-level markers (`#`, `-`, `+`, `>`, ordered-list
            // numbers) are only meaningful at the start of a line and are handled separately
            // when wrapping, so they are intentionally left untouched here.
            '\\' | '`' | '*' | '_' | '[' | ']' | '~' => {
                escaped.push('\\');
                escaped.push(ch);
            }
...
mod tests {
    ...
    fn test_inline_punctuation_is_not_escaped() {
        // Parentheses, hyphens, braces, pipes, and standalone block markers that sit
        // in the middle of a line carry no special meaning and must be left alone.
        let doc = doc(vec![p__(
    ...
    fn test_inline_constructs_are_still_escaped() {
        // Code spans, emphasis, links, and strikethrough can begin anywhere, so their
        // markers are always escaped in plain text.
        let doc = doc(vec![p__("a*b_c`d[e]f~g and \\h")]);
        assert_eq!(write_doc(&doc), "a\\*b\\_c\\`d\\[e\\]f\\~g and \\\\h\n");
    }
